...
function stripEmojis(text: string): string {
  return text.replace(/[\u{1F600}-\u{1F64F}]|[\u{1F300}-\u{1F5FF}]|[\u{1F680}-\u{1F6FF}]|[\u{1F1E0}-\u{1F1FF}]|[\u{2600}-\u{26FF}]|[\u{2700}-\u{27BF}]|[\u{1F900}-\u{1F9FF}]|[\u{1FA00}-\u{1FA6F}]|[\u{1FA70}-\u{1FAFF}]|[\u{231A}-\u{231B}]|[\u{23E9}-\u{23F3}]|[\u{23F8}-\u{23FA}]|[\u{25AA}-\u{25AB}]|[\u{25B6}]|[\u{25C0}]|[\u{25FB}-\u{25FE}]/gu, '').trim();
}

// Concept extraction for semantic deduplication - maps similar ideas to canonical concepts
function extractConcepts(text: string): string[] {
  const conceptMap: Record<string, string[]> = {
    'content': ['post', 'posting', 'content', 'video', 'videos', 'tiktok', 'youtube', 'twitter', 'instagram', 'publish', 'share', 'upload', 'create content'],
    'shipping': ['ship', 'shipping', 'launch', 'deploy', 'release', 'live', 'push'],
    'outreach': ['dm', 'message', 'email', 'outreach', 'cold', 'reach out', 'connect', 'networking'],
    'fitness': ['workout', 'gym', 'exercise', 'run', 'cardio', 'lift', 'training', 'cold shower', 'physical'],
    'discipline': ['wake', 'morning', 'routine', 'daily', 'habit', 'consistency', 'streak'],
    'learning': ['learn', 'study', 'read', 'course', 'skill', 'practice', 'master'],
    'building': ['build', 'code', 'develop', 'feature', 'project', 'app', 'site', 'landing'],
  };
...
Don't default to the same project every time. Make it about THEIR specific life. No generic productivity experiments.` }
    ...
    async function insertAndReturnExperiments(exps: ExperimentOutput[], sprint: SprintConfig): Promise<Response> {
      ...
      for (const exp of exps) {
        const { data: inserted, error: insertError } = await supabase.from("experiments").insert({
          ...
          title: stripEmojis(exp.title),
          description: stripEmojis(exp.description),
          steps: exp.steps.map(s => stripEmojis(s)).join("\n"),
    ...
    try {
      ...
      // SEMANTIC duplicate detection - catches conceptual repeats, not just word overlap
      const newTitle = experiments[0].title.toLowerCase();
      const newDesc = (experiments[0].description || '').toLowerCase();
      const newConcepts = extractConcepts(newTitle + ' ' + newDesc);
      
      // Check against past experiments by CONCEPT, not just words
      const isDuplicate = pastTitles.some((past: string) => {
        const pastConcepts = extractConcepts(past);
        // Count concept overlap (e.g., "posting", "content", "videos" all map to same concept)
        const conceptOverlap = newConcepts.filter(c => pastConcepts.includes(c)).length;
        // Reject if 2+ core concepts overlap OR exact title match
        return conceptOverlap >= 2 || past === newTitle;
      });
      
      if (isDuplicate) {
        console.log("Generated experiment conceptually too similar to past, retrying with different angle");
        // Instead of falling back to static, try regenerating with explicit "don't do this" instruction
        const fallback = getFallbackExperiment(forcedPillar, sprintConfig, userContext);
        return await insertAndReturnExperiments(fallback, sprintConfig);
      }
    } catch (parseError) {
